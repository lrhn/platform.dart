...
void main() {
  ...
  test("Fake Wasm platform", () {
    final currentWasm = WasmPlatform.current;
    final fakeWasm = FakeWasmPlatform(version: "banana");
    fakeWasm.run(() {
      expect(WasmPlatform.current, isNot(same(currentWasm)));
      expect(WasmPlatform.current, same(fakeWasm));
      expect(Platform.current.wasmPlatform, same(fakeWasm));
      expect(WasmPlatform.current?.version, "banana");
    });
  });
}
